...
public:
  HbrMesh(HbrSubdivision<T> *subdivision = 0, int fvarcount = 0,
          const int *fvarindices = 0, const int *fvarwidths = 0,
  ...
  void SetVertexClientData(int id, void *data) {
    if (id >= vertexClientData.size()) {
      ...
      if (s_memStatsIncrement) {
        s_memStatsIncrement((vertexClientData.size() - oldsize) *
                            sizeof(void *));
      }
    }
  ...
  HbrFace<T> *NewFace(int nvertices, HbrVertex<T> **vtx, HbrFace<T> *parent,
                      int childindex);
  ...
  template <typename OutputIterator> void GetFaces(OutputIterator faces) const;
  ...
  void RegisterMemoryRoutines(void (*increment)(unsigned long bytes),
                              void (*decrement)(unsigned long bytes)) {
  ...
  // cost.
  void SetTransientMode(bool mode) { m_transientMode = mode; }
...
template <class T>
HbrMesh<T>::HbrMesh(HbrSubdivision<T> *s, int _fvarcount,
                    const int *_fvarindices, const int *_fvarwidths,
                    int _totalfvarwidth
#ifdef HBRSTITCH
                    ,
                    int _stitchCount
    ...
    : subdivision(s), fvarcount(_fvarcount), fvarindices(_fvarindices),
      ...
      fvarpropagatecorners(false), s_memStatsIncrement(0),
      s_memStatsDecrement(0),
      m_faceSize(sizeof(HbrFace<T>) +
                 4 * ((fvarcount + 15) / 16 * sizeof(unsigned int)
#ifdef HBRSTITCH
      ...
      m_vertexSize(sizeof(HbrVertex<T>) +
                   (totalfvarwidth ? (sizeof(HbrFVarData<T>) +
                                      (totalfvarwidth - 1) * sizeof(float))
                                   : 0)),
      m_vertexAllocator(&m_memory, 512, 0, 0, m_vertexSize),
      m_faceChildrenAllocator(&m_memory, 512, 0, 0), m_memory(0),
      m_numCoarseFaces(-1), hasVertexEdits(0), hasCreaseEdits(0),
      m_transientMode(false) {
}

template <class T> HbrMesh<T>::~HbrMesh() {
  GarbageCollect();
  ...
  for (typename std::vector<HbrHierarchicalEdit<T> *>::iterator hi =
           hierarchicalEdits.begin();
       hi != hierarchicalEdits.end(); ++hi) {
    delete *hi;
  }
}

template <class T> HbrVertex<T> *HbrMesh<T>::NewVertex(int id, const T &data) {
  HbrVertex<T> *v = 0;
  if (nvertices <= id) {
...
template <class T> HbrVertex<T> *HbrMesh<T>::NewVertex(const T &data) {
...
template <class T> HbrVertex<T> *HbrMesh<T>::NewVertex() {
  // Pick an ID - either the maximum vertex ID or a recycled ID if
  // we can
  int id = maxVertexID;
  if (!recycleIDs.empty()) {
...
template <class T>
HbrFace<T> *HbrMesh<T>::NewFace(int nv, const int *vtx, int uindex) {
  HbrVertex<T> **facevertices =
      reinterpret_cast<HbrVertex<T> **>(alloca(sizeof(HbrVertex<T> *) * nv));
  int i;
  for (i = 0; i < nv; ++i) {
  ...
  f->Initialize(this, NULL, -1, maxFaceID, uindex, nv, facevertices,
                totalfvarwidth, 0);
  faces[maxFaceID] = f;
  maxFaceID++;
  // Update the maximum encountered uniform index
  if (uindex > maxUniformIndex)
    maxUniformIndex = uindex;
...
template <class T>
HbrFace<T> *HbrMesh<T>::NewFace(int nv, HbrVertex<T> **vtx, HbrFace<T> *parent,
                                int childindex) {
  HbrFace<T> *f = 0;
  // Resize if needed
  if (nfaces <= maxFaceID) {
  ...
  f->Initialize(this, parent, childindex, maxFaceID,
                parent ? parent->GetUniformIndex() : 0, nv, vtx, totalfvarwidth,
                parent ? parent->GetDepth() + 1 : 0);
  if (parent) {
...
template <class T> void HbrMesh<T>::Finish() {
  int i, j;
  m_numCoarseFaces = 0;
  for (i = 0; i < nfaces; ++i) {
    if (faces[i]) {
  ...
  if (interpboundarymethod == k_InterpolateBoundaryEdgeOnly ||
      interpboundarymethod == k_InterpolateBoundaryEdgeAndCorner) {
    for (i = 0; i < nfaces; ++i) {
  ...
  if (interpboundarymethod == k_InterpolateBoundaryEdgeAndCorner) {
    for (typename std::vector<HbrVertex<T> *>::iterator vi = vertexlist.begin();
         vi != vertexlist.end(); ++vi) {
      HbrVertex<T> *vertex = *vi;
      if (vertex && vertex->IsConnected() && vertex->OnBoundary() &&
          vertex->GetCoarseValence() == 2) {
  ...
  if (!hierarchicalEdits.empty()) {
    ...
    for (i = 0; i < nfaces; ++i) {
      if (faces[i]) {
        while (j < nHierarchicalEdits &&
               hierarchicalEdits[j]->GetFaceID() < i) {
          ++j;
        }
...
template <class T> void HbrMesh<T>::DeleteFace(HbrFace<T> *face) {
...
template <class T> void HbrMesh<T>::DeleteVertex(HbrVertex<T> *vertex) {
...
template <class T> int HbrMesh<T>::GetNumVertices() const {
  int count = 0;
...
template <class T> int HbrMesh<T>::GetNumDisconnectedVertices() const {
  int disconnected = 0;
  for (int i = 0; i < nvertices; ++i) {
...
template <class T> int HbrMesh<T>::GetNumFaces() const {
  int count = 0;
...
template <class T> int HbrMesh<T>::GetNumCoarseFaces() const {
  // Use the value computed by Finish() if it exists
  if (m_numCoarseFaces >= 0)
    return m_numCoarseFaces;
  // Otherwise we have to just count it up now
  int count = 0;
  for (int i = 0; i < nfaces; ++i) {
    if (faces[i] && faces[i]->IsCoarse())
      count++;
  }
  return count;
}

template <class T> HbrFace<T> *HbrMesh<T>::GetFace(int id) const {
  if (id < nfaces) {
    return faces[id];
  }
  return 0;
...
template <class T>
template <typename OutputIterator>
void HbrMesh<T>::GetVertices(OutputIterator lvertices) const {
  for (int i = 0; i < nvertices; ++i) {
    if (vertices[i])
      *lvertices++ = vertices[i];
  }
}

template <class T>
void HbrMesh<T>::ApplyOperatorAllVertices(HbrVertexOperator<T> &op) const {
  for (int i = 0; i < nvertices; ++i) {
    if (vertices[i])
      op(*vertices[i]);
  }
...
template <class T>
template <typename OutputIterator>
void HbrMesh<T>::GetFaces(OutputIterator lfaces) const {
  for (int i = 0; i < nfaces; ++i) {
    if (faces[i])
      *lfaces++ = faces[i];
  }
...
template <class T> void HbrMesh<T>::PrintStats(std::ostream &out) {
  int singular = 0;
  int sumvalence = 0;
  int i, nv = 0;
  int disconnected = 0;
...
template <class T> void HbrMesh<T>::GarbageCollect() {
  if (gcVertices.empty())
    return;

  static const size_t gcthreshold = 4096;

  if (gcVertices.size() <= gcthreshold)
    return;
  ...
  // Delete those faces
  for (typename std::vector<HbrFace<T> *>::iterator fi = killlist.begin();
       fi != killlist.end(); ++fi) {
  ...
  for (typename std::vector<HbrVertex<T> *>::iterator vi = vlist.begin();
       vi != vlist.end(); ++vi) {
    HbrVertex<T> *v = *vi;
    if (!v->IsReferenced()) {
      DeleteVertex(v);
...
template <class T>
void HbrMesh<T>::AddHierarchicalEdit(HbrHierarchicalEdit<T> *edit) {
...
template <class T> void HbrMesh<T>::FreeTransientData() {
  // When purging transient data, we must clear the faces first
  for (typename std::vector<HbrFace<T> *>::iterator fi =
           m_transientFaces.begin();
       fi != m_transientFaces.end(); ++fi) {
    DeleteFace(*fi);
  }
  // The vertices should now be trivial to purge after the transient
  // faces have been cleared
  for (typename std::vector<HbrVertex<T> *>::iterator vi =
           m_transientVertices.begin();
