...
public:
  typedef enum Operation { Set, Add, Subtract } Operation;
...
public:
  virtual ~HbrHierarchicalEdit() { delete[] subfaces; }
  ...
  bool operator<(const HbrHierarchicalEdit &p) const {
    ...
    if (minlength > p.nsubfaces)
      minlength = p.nsubfaces;
    for (int i = 0; i < minlength; ++i) {
      if (subfaces[i] < p.subfaces[i])
        return true;
      if (subfaces[i] > p.subfaces[i])
        return false;
    }
  ...
  virtual void ApplyToBound(struct bbox & /* box */,
                            RtMatrix * /* mx */) const {}
#endif
...
template <class T> class HbrHierarchicalEditComparator {
public:
  bool operator()(const HbrHierarchicalEdit<T> *path1,
                  const HbrHierarchicalEdit<T> *path2) const {
    return (*path1 < *path2);
  }
};
...
template <class T>
bool HbrHierarchicalEdit<T>::IsRelevantToFace(HbrFace<T> *face) const {
  ...
  // we're not relevant
  if (nsubfaces < face->GetDepth())
    return false;

  if (memcmp(subfaces, p->subfaces, face->GetDepth() * sizeof(unsigned char)) !=
      0) {
    return false;
  }
  return true;
