...
public:
  ...
  HbrHalfedge<T> *GetParentEdge() const {
    return (parentType == k_ParentEdge ? parent.edge : 0);
  }
  HbrFace<T> *GetParentFace() const {
    return (parentType == k_ParentFace ? parent.face : 0);
  }
  HbrVertex<T> *GetParentVertex() const {
    return (parentType == k_ParentVertex ? parent.vertex : 0);
  }

  // Set the parent pointer
  void SetParent(HbrHalfedge<T> *edge) {
    assert(!edge || !parent.vertex);
    parentType = k_ParentEdge;
    parent.edge = edge;
  }
  void SetParent(HbrFace<T> *face) {
    assert(!face || !parent.vertex);
    parentType = k_ParentFace;
    parent.face = face;
  }
  void SetParent(HbrVertex<T> *vertex) {
    assert(!vertex || !parent.vertex);
...
HbrVertex<T>::HbrVertex()
    : morefvar(0), id(-1), references(0), used(0), sharpness(0.0f), vchild(-1),
      nIncidentEdges(0), extraordinary(0), validmask(0), volatil(0),
      neighborsguaranteed(0), collected(0), hasvertexedit(0), editsapplied(0),
      destroyed(0), parentType(k_ParentNone) {
  ClearMask();
  parent.vertex = 0;
  incident.edge = 0;
}
...
template <class T>
void HbrVertex<T>::Initialize(int vid, const T &vdata, int fvarwidth) {
  data = vdata;
  morefvar = 0;
  id = vid;
  references = 0;
...
template <class T> HbrVertex<T>::~HbrVertex() { Destroy(); }

template <class T> void HbrVertex<T>::Destroy(HbrMesh<T> *mesh) {
  if (!destroyed) {
...
template <class T> void HbrVertex<T>::AddIncidentEdge(HbrHalfedge<T> *edge) {
...
template <class T> void HbrVertex<T>::RemoveIncidentEdge(HbrHalfedge<T> *edge) {
...
template <class T>
bool HbrVertex<T>::EdgeRemovalWillMakeSingular(HbrHalfedge<T> *edge) const {
...
template <class T> void HbrVertex<T>::Finish() {
  extraordinary = false;
...
template <class T> int HbrVertex<T>::GetValence() const {
  int valence = 0;
  assert(!IsSingular());
  HbrHalfedge<T> *start =
...
template <class T> int HbrVertex<T>::GetCoarseValence() const {
  int valence = 0;
  assert(!IsSingular());
  HbrHalfedge<T> *start =
      (nIncidentEdges > 1) ? incident.edges[0] : incident.edge;
...
template <class T>
HbrFVarData<T> &HbrVertex<T>::GetFVarData(const HbrFace<T> *face) {
  // See if there are any extra facevarying datum associated with
  // this vertex, and whether any of them match the face.
  if (morefvar) {
    size_t fvtsize = sizeof(HbrFVarData<T>) +
                     sizeof(float) * (GetMesh()->GetTotalFVarWidth() - 1);
...
template <class T>
HbrFVarData<T> &HbrVertex<T>::NewFVarData(const HbrFace<T> *face) {
  const int fvarwidth = GetMesh()->GetTotalFVarWidth();
  size_t fvtsize = sizeof(HbrFVarData<T>) + (fvarwidth - 1) * sizeof(float);
  if (morefvar) {
    struct morefvardata *newmorefvar = (struct morefvardata *)malloc(
        sizeof(int) + (morefvar->count + 1) * fvtsize);
    HbrFVarData<T> *newfvt =
        (HbrFVarData<T> *)((char *)newmorefvar + sizeof(int));
...
template <class T> HbrFace<T> *HbrVertex<T>::GetFace() const {
  return GetIncidentEdge()->GetFace();
}

template <class T> HbrMesh<T> *HbrVertex<T>::GetMesh() const {
  return GetFace()->GetMesh();
}

template <class T>
HbrHalfedge<T> *HbrVertex<T>::GetEdge(const HbrVertex<T> *dest) const {
...
template <class T> HbrHalfedge<T> *HbrVertex<T>::GetEdge(int dest) const {
  // Here, we generally want to go through all halfedge cycles
  for (int i = 0; i < nIncidentEdges; ++i) {
    HbrHalfedge<T> *cycle =
        (nIncidentEdges > 1) ? incident.edges[i] : incident.edge;
...
template <class T>
HbrHalfedge<T> *HbrVertex<T>::GetNextEdge(const HbrHalfedge<T> *edge) const {
  // Paranoia:
...
template <class T>
HbrVertex<T> *HbrVertex<T>::GetQEONext(const HbrVertex<T> *dest) const {
...
template <class T>
HbrVertex<T> *HbrVertex<T>::GetQEONext(const HbrHalfedge<T> *edge) const {
  assert(edge->GetOrgVertex() == this);
  return edge->GetPrev()->GetOrgVertex();
}

template <class T>
HbrVertex<T> *HbrVertex<T>::GetQEOPrev(const HbrVertex<T> *dest) const {
  HbrHalfedge<T> *edge = GetEdge(dest);
  if (edge) {
    if (edge->GetOpposite()) {
...
template <class T>
HbrVertex<T> *HbrVertex<T>::GetQEOPrev(const HbrHalfedge<T> *edge) const {
  assert(edge->GetOrgVertex() == this);
  if (edge->GetOpposite()) {
    return edge->GetOpposite()->GetNext()->GetDestVertex();
  } else {
...
template <class T>
HbrVertex<T> *HbrVertex<T>::GetQELNext(const HbrVertex<T> *dest) const {
  HbrHalfedge<T> *edge = GetEdge(dest);
  if (edge) {
    return edge->GetNext()->GetDestVertex();
  }
...
template <class T> bool HbrVertex<T>::OnBoundary() const {
...
template <class T> bool HbrVertex<T>::IsFVarSmooth(int datum) {
  return (GetFVarMask(datum) == k_Smooth);
}

template <class T> bool HbrVertex<T>::IsFVarAllSmooth() {
...
template <class T> bool HbrVertex<T>::IsFVarDart(int datum) {
  return (GetFVarMask(datum) == k_Dart);
}

template <class T> bool HbrVertex<T>::IsFVarCorner(int datum) {
...
template <class T> unsigned char HbrVertex<T>::GetMask(bool next) {
  ...
  while (edge) {
    ...
    if (esharp > HbrHalfedge<T>::k_Smooth &&
        esharp < HbrHalfedge<T>::k_InfinitelySharp) {
      volatil = 1;
    }
    nextedge = GetNextEdge(edge);
    if (nextedge == start) {
...
template <class T> unsigned char HbrVertex<T>::GetFVarMask(int datum) {
...
template <class T> float HbrVertex<T>::GetFractionalMask() const {
  float mask = 0;
  float n = 0;
...
template <class T>
template <typename OutputIterator>
void HbrVertex<T>::GetSurroundingEdges(OutputIterator edges) const {
  HbrHalfedge<T> *start = GetIncidentEdge(), *edge, *next;
  edge = start;
  while (edge) {
    *edges++ = edge;
...
template <class T>
void HbrVertex<T>::ApplyOperatorSurroundingEdges(
    HbrHalfedgeOperator<T> &op) const {
  HbrHalfedge<T> *start = GetIncidentEdge(), *edge, *next;
  edge = start;
  while (edge) {
    op(*edge);
...
template <class T>
template <typename OutputIterator>
void HbrVertex<T>::GetSurroundingVertices(OutputIterator vertices) const {
...
template <class T>
void HbrVertex<T>::ApplyOperatorSurroundingVertices(
    HbrVertexOperator<T> &op) const {
  HbrMesh<T> *mesh = GetMesh();
  HbrHalfedge<T> *start = GetIncidentEdge(), *edge, *next;
  edge = start;
  while (edge) {
...
template <class T>
void HbrVertex<T>::ApplyOperatorSurroundingFaces(HbrFaceOperator<T> &op) const {
...
template <class T> HbrVertex<T> *HbrVertex<T>::Subdivide() {
  HbrMesh<T> *mesh = GetMesh();
  if (vchild != -1)
...
template <class T> void HbrVertex<T>::Refine() {
  HbrMesh<T> *mesh = GetMesh();
  mesh->GetSubdivision()->RefineAtVertex(mesh, this);
}

template <class T> void HbrVertex<T>::GuaranteeNeighbors() {
...
template <class T> unsigned long HbrVertex<T>::GetMemStats() const {
  return sizeof(HbrVertex<T>);
}

template <class T> void HbrVertex<T>::splitSingular() {
  HbrMesh<T> *mesh = GetMesh();
  HbrHalfedge<T> *e;
  HbrHalfedge<T> **incidentEdges =
      (nIncidentEdges > 1) ? incident.edges : &incident.edge;
  ...
  for (int i = 1; i < nIncidentEdges; ++i) {
    ...
    for (typename std::vector<HbrHalfedge<T> *>::iterator ei = edges.begin();
         ei != edges.end(); ++ei) {
      e = *ei;
      if (e->GetOpposite()) {
