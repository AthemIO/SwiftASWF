...
public:
  ...
  // on this item
  void SetWithWeight(const HbrFVarData &fvvi, int startindex, int width,
                     float weight) {
  ...
  // to this item
  void AddWithWeight(const HbrFVarData &fvvi, int startindex, int width,
                     float weight) {
    float *dst = data + startindex;
    const float *src = fvvi.data + startindex;
    for (int i = 0; i < width; ++i) {
  ...
  // if all values match
  bool CompareAll(int width, const float *values,
  ...
  bool Compare(const HbrFVarData &fvvi, int startindex, int width,
               float tolerance = 0.0f) const {
    for (int i = 0; i < width; ++i) {
...
template <class T>
void HbrFVarData<T>::ApplyFVarEdit(const HbrFVarEdit<T> &edit) {
