...
template <class T> class HbrLoopSubdivision : public HbrSubdivision<T> {
public:
  HbrLoopSubdivision<T>() : HbrSubdivision<T>() {}
  ...
  virtual HbrFace<T> *RefineFaceAtVertex(HbrMesh<T> *mesh, HbrFace<T> *face,
                                         HbrVertex<T> *vertex);
  ...
  virtual bool VertexIsExtraordinary(HbrMesh<T> const * /* mesh */,
                                     HbrVertex<T> *vertex) {
    return vertex->GetValence() != 6;
  }
  virtual bool FaceIsExtraordinary(HbrMesh<T> const * /* mesh */,
                                   HbrFace<T> *face) {
    return face->GetNumVertices() != 3;
  }
...
private:
  ...
  void transferFVarToChild(HbrMesh<T> *mesh, HbrFace<T> *face,
                           HbrFace<T> *child, int index);
...
template <class T>
void HbrLoopSubdivision<T>::transferFVarToChild(HbrMesh<T> *mesh,
                                                HbrFace<T> *face,
                                                HbrFace<T> *child, int index) {
  typename HbrMesh<T>::InterpolateBoundaryMethod fvarinterp =
      mesh->GetFVarInterpolateBoundaryMethod();
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      for (int fvaritem = 0; fvaritem < fvarcount; ++fvaritem) {
        ...
        if (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryNone ||
            face->GetEdge(i)->GetFVarSharpness(fvaritem) ||
            face->GetEdge(i)->IsBoundary()) {

          // Sharp edge rule
          fv.SetWithWeight(face->GetFVarData(i), fvarindex, fvarwidth, 0.5f);
          fv.AddWithWeight(face->GetFVarData((i + 1) % 3), fvarindex, fvarwidth,
                           0.5f);
          ...
          fv.AddWithWeight(face->GetFVarData((i + 2) % 3), fvarindex, fvarwidth,
                           0.125f);
          ...
          for (int j = 0; j < oppFace->GetNumVertices(); ++j) {
            if (oppFace->GetVertex(j) == face->GetVertex(i)) {
              fv.AddWithWeight(
                  oppFace->GetFVarData((j + 1) % oppFace->GetNumVertices()),
                  fvarindex, fvarwidth, 0.125f);
              break;
            }
          }
        }
  ...
  for (int fvaritem = 0; fvaritem < fvarcount; ++fvaritem) {
    ...
    if (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryEdgeAndCorner) {
      if (fvarmask >= HbrVertex<T>::k_Corner) {
        ...
        if (face->GetEdge(index)->GetFVarSharpness(fvaritem, true) &&
            face->GetEdge(index)->GetPrev()->GetFVarSharpness(fvaritem, true)) {
          infcorner = true;
        }
      }
    }
    ...
    // - is an infinitely sharp facevarying vertex, in the EDGEANDCORNER case;
    // or
    ...
    if (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryNone ||
        (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryAlwaysSharp &&
         fvarmask >= 1) ||
        v->GetSharpness() > HbrVertex<T>::k_Smooth || infcorner) {
    ...
    // Smooth rule
      ...
      fv0.SetWithWeight(face->GetFVarData(index), fvarindex, fvarwidth,
                        1 - (beta * valence));
      ...
      while (edge) {
        ...
        for (int j = 0; j < g->GetNumVertices(); ++j) {
          if (g->GetEdge(j)->GetOrgVertex() == v) {
            fv0.AddWithWeight(g->GetFVarData((j + 1) % g->GetNumVertices()),
                              fvarindex, fvarwidth, beta);
            break;
          }
        }
    ...
    if (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryNone ||
      ...
      fv1.AddWithWeight(face->GetFVarData((index + 1) % 3), fvarindex,
                        fvarwidth, 0.5f);
      ...
      fv1.AddWithWeight(face->GetFVarData((index + 1) % 3), fvarindex,
                        fvarwidth, 0.375f);
      // Add 0.125 of opposite vertices
      fv1.AddWithWeight(face->GetFVarData((index + 2) % 3), fvarindex,
                        fvarwidth, 0.125f);
    ...
    if (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryNone ||
      ...
      // Sharp edge rule
      fv2.SetWithWeight(face->GetFVarData((index + 2) % 3), fvarindex,
                        fvarwidth, 0.5f);
      ...
      // Add 0.125 of opposite vertices
      fv2.AddWithWeight(face->GetFVarData((index + 1) % 3), fvarindex,
                        fvarwidth, 0.125f);
      ...
      for (int j = 0; j < oppFace->GetNumVertices(); ++j) {
        if (oppFace->GetVertex(j) == v) {
          fv2.AddWithWeight(
              oppFace->GetFVarData((j + 2) % oppFace->GetNumVertices()),
              fvarindex, fvarwidth, 0.125f);
          break;
        }
      }
...
template <class T>
void HbrLoopSubdivision<T>::transferEditsToChild(HbrFace<T> *face,
                                                 HbrFace<T> *child, int index) {
...
template <class T>
void HbrLoopSubdivision<T>::Refine(HbrMesh<T> *mesh, HbrFace<T> *face) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (!face->GetChild(i)) {
      ...
      if ((sharpness = edge->GetSharpness()) > HbrHalfedge<T>::k_Smooth) {
        HbrSubdivision<T>::SubdivideCreaseWeight(edge, edge->GetOrgVertex(),
                                                 childedge);
      }
...
template <class T>
HbrFace<T> *HbrLoopSubdivision<T>::RefineFaceAtVertex(HbrMesh<T> *mesh,
                                                      HbrFace<T> *face,
                                                      HbrVertex<T> *vertex) {
...
template <class T>
void HbrLoopSubdivision<T>::GuaranteeNeighbor(HbrMesh<T> *mesh,
                                              HbrHalfedge<T> *edge) {
  if (edge->GetOpposite()) {
    return;
  }
...
template <class T>
void HbrLoopSubdivision<T>::GuaranteeNeighbors(HbrMesh<T> *mesh,
                                               HbrVertex<T> *vertex) {
...
template <class T>
bool HbrLoopSubdivision<T>::HasLimit(HbrMesh<T> *mesh, HbrFace<T> *face) {
...
template <class T>
bool HbrLoopSubdivision<T>::HasLimit(HbrMesh<T> *mesh, HbrHalfedge<T> *edge) {
  ...
  if (!HasLimit(mesh, edge->GetOrgVertex()) ||
      !HasLimit(mesh, edge->GetDestVertex()))
    return false;
...
template <class T>
bool HbrLoopSubdivision<T>::HasLimit(HbrMesh<T> * /* mesh */,
                                     HbrVertex<T> *vertex) {
  ...
  default:
    if (vertex->IsVolatile()) {
      ...
      while (edge) {
        if (edge->IsBoundary() &&
            edge->GetSharpness() < HbrHalfedge<T>::k_InfinitelySharp) {
          return false;
        }
...
template <class T>
HbrVertex<T> *HbrLoopSubdivision<T>::Subdivide(HbrMesh<T> * /* mesh */,
                                               HbrFace<T> * /* face */) {
...
HbrVertex<T> *HbrLoopSubdivision<T>::Subdivide(HbrMesh<T> *mesh,
  ...
  if (!edge->IsBoundary() && esharp <= 1.0f) {
    ...
    if (edge->GetOpposite() && edge->GetOpposite()->GetFace()->GetPath() <
                                   edge->GetFace()->GetPath()) {
...
HbrVertex<T> *HbrLoopSubdivision<T>::Subdivide(HbrMesh<T> *mesh,
  ...
  for (int i = 0; i < passes; ++i) {
    ...
    case HbrVertex<T>::k_Dart: {
      ...
      data.AddWithWeight(vertex->GetData(),
                         weights[i] * (1 - (beta * valence)));
    ...
    case HbrVertex<T>::k_Crease: {
      ...
      // sharp edges
      HbrSubdivision<T>::AddCreaseEdgesWithWeight(mesh, vertex, i == 1,
                                                  weights[i] * 0.125f, &data);
      break;
    }
    case HbrVertex<T>::k_Corner:
    default: {
...
template <class T>
void HbrLoopSubdivision<T>::refineFaceAtMiddle(HbrMesh<T> *mesh,
                                               HbrFace<T> *face) {
