...
template <class T> class HbrCatmarkSubdivision : public HbrSubdivision<T> {
public:
  ...
  virtual bool VertexIsExtraordinary(HbrMesh<T> const * /* mesh */,
                                     HbrVertex<T> *vertex) {
    return vertex->GetValence() != 4;
  }
  virtual bool FaceIsExtraordinary(HbrMesh<T> const * /* mesh */,
                                   HbrFace<T> *face) {
    return face->GetNumVertices() != 4;
  }
...
template <class T>
void HbrCatmarkSubdivision<T>::transferFVarToChild(HbrMesh<T> *mesh,
                                                   HbrFace<T> *face,
                                                   HbrFace<T> *child,
                                                   int index) {
  ...
  for (int fvaritem = 0; fvaritem < fvarcount; ++fvaritem) {
    ...
    // computed the facevarying values
      ...
      fv0.SetWithWeight(face->GetFVarData(index), fvarindex, fvarwidth,
                        invvalencesquared * valence * (valence - 2));
        ...
        for (int j = 0; j < g->GetNumVertices(); ++j) {
          ...
          if (g->GetEdge(j)->GetOrgVertex() == v) {
            fv0.AddWithWeight(g->GetFVarData((j + 1) % g->GetNumVertices()),
                              fvarindex, fvarwidth, invvalencesquared);
          }
        }
    ...
    if (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryNone ||
      ...
      fv1.AddWithWeight(face->GetFVarData((index + 1) % nv), fvarindex,
                        fvarwidth, 0.5f);
      ...
      for (int j = 0; j < oppFace->GetNumVertices(); ++j) {
        fv1.AddWithWeight(oppFace->GetFVarData(j), fvarindex, fvarwidth,
                          weight);
      }
    }
    ...
    if (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryNone ||
      ...
      // Sharp edge rule
      fv3.SetWithWeight(face->GetFVarData((index + nv - 1) % nv), fvarindex,
                        fvarwidth, 0.5f);
      ...
      weight = 0.25f / oppFace->GetNumVertices();
      for (int j = 0; j < oppFace->GetNumVertices(); ++j) {
        fv3.AddWithWeight(oppFace->GetFVarData(j), fvarindex, fvarwidth,
                          weight);
      }
    }
...
template <class T>
void HbrCatmarkSubdivision<T>::transferEditsToChild(HbrFace<T> *face,
                                                    HbrFace<T> *child,
                                                    int index) {
...
template <class T>
void HbrCatmarkSubdivision<T>::Refine(HbrMesh<T> *mesh, HbrFace<T> *face) {
...
template <class T>
HbrFace<T> *HbrCatmarkSubdivision<T>::RefineFaceAtVertex(HbrMesh<T> *mesh,
                                                         HbrFace<T> *face,
                                                         HbrVertex<T> *vertex) {
...
template <class T>
void HbrCatmarkSubdivision<T>::GuaranteeNeighbor(HbrMesh<T> *mesh,
                                                 HbrHalfedge<T> *edge) {
  if (edge->GetOpposite()) {
    return;
  }
...
template <class T>
void HbrCatmarkSubdivision<T>::GuaranteeNeighbors(HbrMesh<T> *mesh,
                                                  HbrVertex<T> *vertex) {
...
template <class T>
bool HbrCatmarkSubdivision<T>::HasLimit(HbrMesh<T> *mesh, HbrFace<T> *face) {
...
template <class T>
bool HbrCatmarkSubdivision<T>::HasLimit(HbrMesh<T> *mesh,
                                        HbrHalfedge<T> *edge) {
...
template <class T>
HbrVertex<T> *HbrCatmarkSubdivision<T>::Subdivide(HbrMesh<T> *mesh,
                                                  HbrFace<T> *face) {
...
HbrVertex<T> *HbrCatmarkSubdivision<T>::Subdivide(HbrMesh<T> *mesh,
  ...
  if (!edge->IsBoundary() && esharp <= 1.0f) {
    ...
    leftWeight = (triangleSubdivision == k_New && lf->GetNumVertices() == 3)
                     ? HBR_SMOOTH_TRI_EDGE_WEIGHT
                     : 0.25f;
    rightWeight = (triangleSubdivision == k_New && rf->GetNumVertices() == 3)
                      ? HBR_SMOOTH_TRI_EDGE_WEIGHT
                      : 0.25f;
...
HbrVertex<T> *HbrCatmarkSubdivision<T>::Subdivide(HbrMesh<T> *mesh,
  ...
  for (int i = 0; i < passes; ++i) {
    ...
    case HbrVertex<T>::k_Dart: {
      ...
      data.AddWithWeight(vertex->GetData(), weights[i] * invvalencesquared *
                                                valence * (valence - 2));
      ...
      while (edge) {
        HbrFace<T> *f = edge->GetLeftFace();
        data.AddWithWeight(f->Subdivide()->GetData(),
                           weights[i] * invvalencesquared);
