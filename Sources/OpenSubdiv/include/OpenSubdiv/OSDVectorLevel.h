...
//  those that are purely quads or tris -- so we prefer to insulate it from
//  public access.
//
//  The representation of topology here is to store six topological
//  relationships in tables of integers.  Each is stored in its own array(s) so
//  the result is a SOA representation of the topology.  The six relations are:
//
...
public:
  ...
  struct VTag {
    ...
    //  directly:
    explicit VTag(VTagSize bits) { std::memcpy(this, &bits, sizeof(bits)); }
  ...
  struct ETag {
    ...
    //  Alternate constructor and accessor for dealing with integer bits
    //  directly:
    explicit ETag(ETagSize bits) { std::memcpy(this, &bits, sizeof(bits)); }
...
public:
  ...
  //  Methods to access the relation tables/indices -- note that for some
  //  relations (i.e. those where a component is "contained by" a neighbor, or
  //  more generally when the neighbor is a simplex of higher dimension) we
  //  store an additional "local index", e.g. for the case of vert-faces if one
  //  of the faces F[i] is incident a vertex V, then L[i] is the "local index"
  //  in F[i] of vertex V. Once have only quads (or tris), this local index need
  //  only occupy two bits and could conceivably be packed into the same integer
  //  as the face index, but for now, given the need to support faces of
  //  potentially high valence we'll use an 8- or 16-bit integer.
  //
  ...
  //  Replace these with access to sharpness buffers/arrays rather than
  //  elements:
...
public:
  ...
  typedef void (*ValidationCallback)(TopologyError errCode, char const *msg,
                                     void const *clientData);

  bool validateTopology(ValidationCallback callback = 0,
                        void const *clientData = 0) const;
...
public:
  ...
  bool isSingleCreasePatch(Index face, float *sharpnessOut = NULL,
                           int *rotationOut = NULL) const;

  //
  //  When inspecting topology, the component tags -- particularly VTag and ETag
  //  -- are most often inspected in groups for the face to which they belong.
  //  They are designed to be bitwise OR'd (the result then referred to as a
  //  "composite" tag) to make quick decisions about the face as a whole to
  //  avoid tedious topological inspection.
  //
  //  The same logic can be applied to topology in a FVar channel when tags
  //  specific to that channel are used.  Note that the VTags apply to the FVar
  //  values assigned to the corners of the face and not the vertex as a whole.
  //  The "composite" face-varying VTag for a vertex is the union of VTags of
  //  all distinct FVar values for that vertex.
  //
  ...
  //
  //  When gathering "patch points" we may want the indices of the vertices or
  //  the corresponding FVar values for a particular channel.  Both are
  //  represented and equally accessible within the faces, so we allow all to be
  //  returned through these methods.  Setting the optional FVar channel to -1
  //  will retrieve indices of vertices instead of FVar values:
  //
  int gatherQuadLinearPatchPoints(Index fIndex, Index patchPoints[],
                                  int rotation = 0, int fvarChannel = -1) const;

  int gatherQuadRegularInteriorPatchPoints(Index fIndex, Index patchPoints[],
                                           int rotation = 0,
                                           int fvarChannel = -1) const;
  int gatherQuadRegularBoundaryPatchPoints(Index fIndex, Index patchPoints[],
                                           int boundaryEdgeInFace,
                                           int fvarChannel = -1) const;
  int gatherQuadRegularCornerPatchPoints(Index fIndex, Index patchPoints[],
                                         int cornerVertInFace,
                                         int fvarChannel = -1) const;

  int gatherQuadRegularRingAroundVertex(Index vIndex, Index ringPoints[],
                                        int fvarChannel = -1) const;
  int gatherQuadRegularPartialRingAroundVertex(Index vIndex, VSpan const &span,
                                               Index ringPoints[],
                                               int fvarChannel = -1) const;

  //  WIP -- for future use, need to extend for face-varying...
  int gatherTriRegularInteriorPatchPoints(Index fIndex, Index patchVerts[],
                                          int rotation = 0) const;
  int gatherTriRegularBoundaryVertexPatchPoints(Index fIndex,
                                                Index patchVerts[],
                                                int boundaryVertInFace) const;
  int gatherTriRegularBoundaryEdgePatchPoints(Index fIndex, Index patchVerts[],
                                              int boundaryEdgeInFace) const;
  int gatherTriRegularCornerVertexPatchPoints(Index fIndex, Index patchVerts[],
                                              int cornerVertInFace) const;
  int gatherTriRegularCornerEdgePatchPoints(Index fIndex, Index patchVerts[],
...
public:
  ...
  //      - these may be unwarranted if we let Refinement access members
  //      directly...
  ...
  //
  //  Note that for some relations, the size of the relations for a child
  //  component can vary radically from its parent due to the sparsity of the
  //  refinement.  So in these cases a few additional utilities are provided to
  //  help define the set of incident components.  Assuming adequate memory has
  //  been allocated, the "resize" methods here initialize the set of incident
  //  components by setting both the size and the appropriate offset, while
  //  "trim" is use to quickly lower the size from an upper bound and nothing
  //  else.
  //
...
public:
  ...
  bool orderVertexFacesAndEdges(Index vIndex, Index *vFaces,
                                Index *vEdges) const;
...
private:
  ...
  //
  //  A Level is independent of subdivision scheme or options.  While it may
  //  have been affected by them in its construction, they are not associated
  //  with it -- a Level is pure topology and any subdivision parameters are
  //  external.
  //
  ...
  //  The "depth" member is clearly useful in both the topological splitting and
  //  the stencil queries, but arguably it ties the Level to a hierarchy which
  //  counters the idea of it being independent.
  int _depth;

  //  Maxima to help clients manage sizing of data buffers.  Given "max
  //  valence", the "max edge faces" is strictly redundant as it will always be
  //  less, but since it will typically be so much less (i.e. 2) it is kept for
  //  now.
  int _maxEdgeFaces;
  int _maxValence;
  ...
  //      stored in the osd::FarTables in any form.  The FarTable vectors
  //      combine the edge-vert and edge-face relations.  The eventual goal is
  //      that this data be part of the osd::Far classes and be a superset of
  //      the FarTable vectors, i.e. no data duplication or conversion.  The
  //      fact that FarTable already stores 5 of the 6 possible relations should
  //      make the topology storage as a whole a non-issue.
  //
  //      The vert-face-child and vert-edge-child indices are also arguably not
  //      a topology relation but more one for parent/child relations.  But it
  //      is a topological relationship, and if named differently would not
  //      likely raise this.  It has been named with "child" in the name as it
  //      does play a more significant role during subdivision in mapping
  //      between parent and child components, and so has been named to reflect
  //      that more clearly.
  //

  //  Per-face:
  std::vector<Index>
      _faceVertCountsAndOffsets;       // 2 per face, redundant after level 0
  ...
  std::vector<LocalIndex>
      _vertFaceLocalIndices; // varies with valence, 8-bit for now

  std::vector<Index> _vertEdgeCountsAndOffsets; // 2 per vertex
  std::vector<Index> _vertEdgeIndices;          // varies with valence
  std::vector<LocalIndex>
      _vertEdgeLocalIndices; // varies with valence, 8-bit for now
...
//
inline ConstIndexArray Level::getFaceVertices(Index faceIndex) const {
  return ConstIndexArray(
      &_faceVertIndices[_faceVertCountsAndOffsets[faceIndex * 2 + 1]],
      _faceVertCountsAndOffsets[faceIndex * 2]);
}
inline IndexArray Level::getFaceVertices(Index faceIndex) {
  return IndexArray(
      &_faceVertIndices[_faceVertCountsAndOffsets[faceIndex * 2 + 1]],
      _faceVertCountsAndOffsets[faceIndex * 2]);
}

inline void Level::resizeFaceVertices(Index faceIndex, int count) {

  int *countOffsetPair = &_faceVertCountsAndOffsets[faceIndex * 2];

  countOffsetPair[0] = count;
  countOffsetPair[1] =
      (faceIndex == 0) ? 0 : (countOffsetPair[-2] + countOffsetPair[-1]);

  _maxValence = std::max(_maxValence, count);
}

inline ConstIndexArray Level::getFaceVertices() const {
...
//
inline ConstIndexArray Level::getFaceEdges(Index faceIndex) const {
  return ConstIndexArray(
      &_faceEdgeIndices[_faceVertCountsAndOffsets[faceIndex * 2 + 1]],
      _faceVertCountsAndOffsets[faceIndex * 2]);
}
inline IndexArray Level::getFaceEdges(Index faceIndex) {
  return IndexArray(
      &_faceEdgeIndices[_faceVertCountsAndOffsets[faceIndex * 2 + 1]],
      _faceVertCountsAndOffsets[faceIndex * 2]);
}
...
//
inline ConstIndexArray Level::getVertexFaces(Index vertIndex) const {
  return ConstIndexArray((&_vertFaceIndices[0]) +
                             _vertFaceCountsAndOffsets[vertIndex * 2 + 1],
                         _vertFaceCountsAndOffsets[vertIndex * 2]);
}
inline IndexArray Level::getVertexFaces(Index vertIndex) {
  return IndexArray((&_vertFaceIndices[0]) +
                        _vertFaceCountsAndOffsets[vertIndex * 2 + 1],
                    _vertFaceCountsAndOffsets[vertIndex * 2]);
}
...
Level::getVertexFaceLocalIndices(Index vertIndex) const {
  return ConstLocalIndexArray((&_vertFaceLocalIndices[0]) +
                                  _vertFaceCountsAndOffsets[vertIndex * 2 + 1],
                              _vertFaceCountsAndOffsets[vertIndex * 2]);
}
inline LocalIndexArray Level::getVertexFaceLocalIndices(Index vertIndex) {
  return LocalIndexArray((&_vertFaceLocalIndices[0]) +
                             _vertFaceCountsAndOffsets[vertIndex * 2 + 1],
                         _vertFaceCountsAndOffsets[vertIndex * 2]);
}

inline void Level::resizeVertexFaces(Index vertIndex, int count) {
  int *countOffsetPair = &_vertFaceCountsAndOffsets[vertIndex * 2];

  countOffsetPair[0] = count;
  countOffsetPair[1] =
      (vertIndex == 0) ? 0 : (countOffsetPair[-2] + countOffsetPair[-1]);
}
inline void Level::trimVertexFaces(Index vertIndex, int count) {
...
//
inline ConstIndexArray Level::getVertexEdges(Index vertIndex) const {
  return ConstIndexArray((&_vertEdgeIndices[0]) +
                             _vertEdgeCountsAndOffsets[vertIndex * 2 + 1],
                         _vertEdgeCountsAndOffsets[vertIndex * 2]);
}
inline IndexArray Level::getVertexEdges(Index vertIndex) {
  return IndexArray((&_vertEdgeIndices[0]) +
                        _vertEdgeCountsAndOffsets[vertIndex * 2 + 1],
                    _vertEdgeCountsAndOffsets[vertIndex * 2]);
}
...
Level::getVertexEdgeLocalIndices(Index vertIndex) const {
  return ConstLocalIndexArray((&_vertEdgeLocalIndices[0]) +
                                  _vertEdgeCountsAndOffsets[vertIndex * 2 + 1],
                              _vertEdgeCountsAndOffsets[vertIndex * 2]);
}
inline LocalIndexArray Level::getVertexEdgeLocalIndices(Index vertIndex) {
  return LocalIndexArray((&_vertEdgeLocalIndices[0]) +
                             _vertEdgeCountsAndOffsets[vertIndex * 2 + 1],
                         _vertEdgeCountsAndOffsets[vertIndex * 2]);
}

inline void Level::resizeVertexEdges(Index vertIndex, int count) {
...
inline void Level::trimVertexEdges(Index vertIndex, int count) {
  _vertEdgeCountsAndOffsets[vertIndex * 2] = count;
}

inline void Level::setMaxValence(int valence) { _maxValence = valence; }

//
//  Access/modify the vertices incident a given edge:
//
inline ConstIndexArray Level::getEdgeVertices(Index edgeIndex) const {
  return ConstIndexArray(&_edgeVertIndices[edgeIndex * 2], 2);
}
inline IndexArray Level::getEdgeVertices(Index edgeIndex) {
...
inline LocalIndexArray Level::getEdgeFaceLocalIndices(Index edgeIndex) {
  return LocalIndexArray(&_edgeFaceLocalIndices[0] +
                             _edgeFaceCountsAndOffsets[edgeIndex * 2 + 1],
                         _edgeFaceCountsAndOffsets[edgeIndex * 2]);
}

inline void Level::resizeEdgeFaces(Index edgeIndex, int count) {
  int *countOffsetPair = &_edgeFaceCountsAndOffsets[edgeIndex * 2];

  countOffsetPair[0] = count;
  countOffsetPair[1] =
      (edgeIndex == 0) ? 0 : (countOffsetPair[-2] + countOffsetPair[-1]);

  _maxEdgeFaces = std::max(_maxEdgeFaces, count);
}
inline void Level::trimEdgeFaces(Index edgeIndex, int count) {
...
//
inline float Level::getEdgeSharpness(Index edgeIndex) const {
  return _edgeSharpness[edgeIndex];
}
inline float &Level::getEdgeSharpness(Index edgeIndex) {
  return _edgeSharpness[edgeIndex];
}

inline float Level::getVertexSharpness(Index vertIndex) const {
  return _vertSharpness[vertIndex];
}
inline float &Level::getVertexSharpness(Index vertIndex) {
  return _vertSharpness[vertIndex];
}
...
//
inline void Level::setFaceHole(Index faceIndex, bool b) {
  _faceTags[faceIndex]._hole = b;
}
inline bool Level::isFaceHole(Index faceIndex) const {
...
//
inline void Level::setEdgeNonManifold(Index edgeIndex, bool b) {
  _edgeTags[edgeIndex]._nonManifold = b;
}
inline bool Level::isEdgeNonManifold(Index edgeIndex) const {
  return _edgeTags[edgeIndex]._nonManifold;
}

inline void Level::setVertexNonManifold(Index vertIndex, bool b) {
  _vertTags[vertIndex]._nonManifold = b;
}
inline bool Level::isVertexNonManifold(Index vertIndex) const {
...
//
inline void Level::resizeFaces(int faceCount) {
...
inline void Level::resizeFaceVertices(int totalFaceVertCount) {
  _faceVertIndices.resize(totalFaceVertCount);
}
inline void Level::resizeFaceEdges(int totalFaceEdgeCount) {
  _faceEdgeIndices.resize(totalFaceEdgeCount);
}

inline void Level::resizeEdges(int edgeCount) {
...
inline void Level::resizeEdgeVertices() {

  _edgeVertIndices.resize(2 * _edgeCount);
}
inline void Level::resizeEdgeFaces(int totalEdgeFaceCount) {

  _edgeFaceIndices.resize(totalEdgeFaceCount);
  _edgeFaceLocalIndices.resize(totalEdgeFaceCount);
}

inline void Level::resizeVertices(int vertCount) {
